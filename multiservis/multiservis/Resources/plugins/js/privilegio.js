...
function Nuevo() {
    ...
    var codigo = '<p class="red-text text-darken-3 flow-text">CREAR NUEVO PRIVILEGIO</p>';
...
function EvaluarVacios() {
    ...
    if ($('#selectRol').val() == null) {
        Materialize.toast('Debe seleccionar un Rol!', 8000);
        return false;
    }
    else {
        return true;
...
function Guardar() {
    var i = $('#id').val();
    var nom = $('#nombre').val();
    var rolP = $('#selectRol').val();
    $.getJSON("/Privilegio/Guardar", { id: i, rol: rolP, nombre: nom, estado: est }, function (e) {
        if (e != "") {
...
function Editar(id) {
    var o = { id: id };
    $.getJSON("/Privilegio/Get", o, function (obj) {
...
function Listar() {
    $.getJSON("/Privilegio/Listar", null, function (cadena) {
        $("#tabla").html(cadena);
        CrearDataTable();
    });
    $('#btnListar').show();
...
$('#aceptarEliminar').click(function () {
    ...
    Materialize.toast('El privilegio fue eliminado exitosamente!', 8000);
    Listar();
});
...
function Eliminar(id) {
    var o = { id: id };
    $.getJSON("/Privilegio/Delete", o, function (e) {
        Listar();
    });
};
